 ...
 * @notice Marketplace for AI agent services with integrated escrow
 * @dev Allows agents to list services and customers to place orders with escrow payments
 */
contract ServiceMarketplace is Ownable, ReentrancyGuard {
     ...
     */
    modifier onlyAgent() {
        require(agentRegistry.checkRegistration(msg.sender), "ServiceMarketplace: caller not a registered agent");
        _;
    }
    ...
    constructor(address _agentRegistry, address _escrow) {
        ...
        require(_escrow != address(0), "ServiceMarketplace: invalid escrow address");
     ...
     * @dev Buyer must first call escrow.deposit{value: price}(orderId, agent) to lock funds
     * @return Order struct
     */
    function placeOrder(bytes32 serviceId, bytes32 orderId) external nonReentrant returns (Order memory) {
        ...
        // Verify buyer has deposited funds in escrow
        require(escrow.hasActiveEscrow(orderId), "ServiceMarketplace: no escrow deposit");
    ...
    /**
     * @notice Mark order as completed and release escrow to agent (agent only)
     * @param orderId Order ID
     */
    function completeOrder(bytes32 orderId) external onlyAgent {
        Order storage order = orders[orderId];
        require(order.agent == msg.sender, "ServiceMarketplace: not authorized");
        ...
        // Update agent reputation positively (+10 per completed order)
        agentRegistry.updateReputation(order.agent, 10);

        emit OrderCompleted(orderId);
    }

    /**
     * @notice Cancel an order and refund buyer
     * @param orderId Order ID
     */
